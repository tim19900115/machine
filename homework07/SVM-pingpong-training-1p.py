...
#----------------------------------------------------------------------------------------------------------------------------
import numpy as np
Plat1X=np.array(platform_1P)[:,0][:,np.newaxis]
Plat1X_next=Plat1X[1:,:]
#Plat1Y=np.array(platform_1P)[:,0][:,np.newaxis]

instruct1=(Plat1X_next-Plat1X[0:len(Plat1X_next),0][:,np.newaxis])/5
...
#特徵球x、y 平板x 球vx vy
x=np.hstack((Ballarray, Plat1X[0:-1,0][:,np.newaxis],VX,VY))
print(len(Plat1X[0:-1,0][:,np.newaxis]))
print(x)
y=instruct1
